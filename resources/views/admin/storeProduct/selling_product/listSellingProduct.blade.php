...
{{-- @yield('title', 'Product') --}}
@include('admin.storeProduct.selling_product.crud_modal.addSellingProductModal')
@include('admin.storeProduct.selling_product.crud_modal.editSellingProductModal')
@section('content')
    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light"> <a href="{{ route('admin.home.listHome') }}"> Home
                /</a></span>Store Selling Products</h4>
            ...
            <div class="m-3 d-flex gap-2">
                ...
                <div>
                    <button type="button" class="btn btn-primary" id="addModalBtn" data-bs-toggle="modal"
                        data-bs-target="#addSellingProductModal">
                ...
                <thead class="text-center">
                    <tr>
                        <th><a class="text-dark"
                                href="{{ route('admin.storeProduct.listSellingProduct', ['sort' => 'created_at', 'direction' => $sortColumn == 'created_at' && $sortDirection == 'asc' ? 'desc' : 'asc']) }}">Created
                        ...
                        <th><a class="text-dark"
                                href="{{ route('admin.storeProduct.listSellingProduct', ['sort' => 'id', 'direction' => $sortColumn == 'id' && $sortDirection == 'asc' ? 'desc' : 'asc']) }}">
                                Product id
                                @if ($sortColumn == 'id')
                                    @if ($sortDirection == 'asc')
                                        <i class="fas fa-arrow-up"></i>
                        ...
                        <th><a class="text-dark"
                                href="{{ route('admin.storeProduct.listSellingProduct', ['sort' => 'name', 'direction' => $sortColumn == 'name' && $sortDirection == 'asc' ? 'desc' : 'asc']) }}">
                                Product name
                                @if ($sortColumn == 'name')
                                    @if ($sortDirection == 'asc')
                                        <i class="fas fa-arrow-up"></i>
                        ...
                        <th><a class="text-dark"
                                href="{{ route('admin.storeProduct.listSellingProduct', ['sort' => 'user_id', 'direction' => $sortColumn == 'user_id' && $sortDirection == 'asc' ? 'desc' : 'asc']) }}">
                                Store id
                        ...
                        <th><a class="text-dark"
                                href="{{ route('admin.storeProduct.listSellingProduct', ['sort' => 'condition', 'direction' => $sortColumn == 'condition' && $sortDirection == 'asc' ? 'desc' : 'asc']) }}">
                                condition
                        ...
                        <th><a class="text-dark"
                                href="{{ route('admin.storeProduct.listSellingProduct', ['sort' => 'price', 'direction' => $sortColumn == 'price' && $sortDirection == 'asc' ? 'desc' : 'asc']) }}">
                                price
                                @if ($sortColumn == 'price')
                                    @if ($sortDirection == 'asc')
                                        <i class="fas fa-arrow-up"></i>
                        ...
                        <th>Add Images</th>
                        <th><a class="text-dark"
                                href="{{ route('admin.storeProduct.listSellingProduct', ['sort' => 'money_collection', 'direction' => $sortColumn == 'money_collection' && $sortDirection == 'asc' ? 'desc' : 'asc']) }}">
                                money_collection
                ...
                <tbody class="text-center">
                    @foreach ($listSellingProduct as $key => $sellingProduct)
                        <tr>
                            ...
                            <td><button type="button" value="{{ $sellingProduct->id }}"
                                    class="edit_sellingProduct  btn btn-primary editbtn btn-sm ">Edit</button></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
    ...
    <script>
        $(document).ready(function() {
            $('#search').on('keyup', function() {
                ...
                console.log(z)
                if (z.length < 1) {
                    location.href = 'storeSellingProduct'
                }
            });
            $('.addSelect').select2({
                width: '100%',
                dropdownParent: $('#addSellingProductModal'),
                theme: 'classic'
            })
            $('.editSelect').select2({
                width: '100%',
                dropdownParent: $('#editSellingProductModal'),
                theme: 'classic'
            })
        ...
        //                 <button type="button" value="${data.id}" class="edit_sellingProduct  btn btn-primary editbtn btn-sm "><i class="fa fa-edit"></i></button>`;
            ...
            //Add Product
            $(document).on('click', '.add_selling_product', function(e) {
                e.preventDefault();
                var data = {
                    'store': $('.store').val(),
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.status == 400) {
                            ...
                            store == '' ? $('#error_edit_selling_store').html(response.errors
                                .store) : $('#error_edit_selling_store').html('')
                            ...
                            description == '' ? $('#error_edit_selling_description').html(
                                    response
