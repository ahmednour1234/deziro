...
@section('content')
    <h4 class="py-3 mb-4">
        <span class="text-muted fw-light">
            ...
            Attributes </a> / Edit {{ $attribute->name }}
    </h4>
    ...
    <div class="card">
        ...
        <div class="modal-body">
            ...
            <form method="POST" action="{{ route('admin.attributes.update', $attribute->id) }}" @submit.prevent="onSubmit"
                ...
                <div class="page-content">
                    <div class="form-container">
                                    ...
                                    <div class="col-12 mb-2">
                                        <div class="control-group">
                                            ...
                                            <select class="form-control select2 category_type" id="type" name="type"
                                                required>
                                                {{-- <option value="text"
                                                    {{ old('type', $attribute->type) == 'text' ? 'selected' : '' }}>
                                                    {{ __('Text') }}</option>
                                                <option value="textarea"
                                                    {{ old('type', $attribute->type) == 'textarea' ? 'selected' : '' }}>
                                                    {{ __('Textarea') }}</option>
                                                <option value="price"
                                                    {{ old('type', $attribute->type) == 'price' ? 'selected' : '' }}>
                                                    {{ __('Price') }}</option>
                                                <option value="boolean"
                                                    {{ old('type', $attribute->type) == 'boolean' ? 'selected' : '' }}>
                                                    {{ __('Boolean') }}</option> --}}
                                                <option value="select"
                                                ...
                                                <option value="datetime"
                                                    {{ old('type', $attribute->type) == 'datetime' ? 'selected' : '' }}>
                                                    {{ __('Datetime') }}</option>
                                                <option value="date"
                                                    {{ old('type', $attribute->type) == 'date' ? 'selected' : '' }}>
                                                    {{ __('Date') }}</option>
                                                <option value="image"
                                                    {{ old('type', $attribute->type) == 'image' ? 'selected' : '' }}>
                                                    {{ __('Image') }}</option>
                                                <option value="file"
                                                    {{ old('type', $attribute->type) == 'file' ? 'selected' : '' }}>
                                                    {{ __('File') }}</option>
                                                <option value="checkbox"
                                                    {{ old('type', $attribute->type) == 'checkbox' ? 'selected' : '' }}>
                                                    {{ __('Checkbox') }}</option> --}}
                                            </select>
                                            @if ($errors->has('type'))
                        ...
                        <div
                            class="{{ in_array(old('type', $attribute->type), ['select', 'multiselect', 'checkbox']) ? '' : 'hide' }}">
                            <section id="options">
                                <h5 class="section-header">Options</h5>
                                <div class="section-body">
                                    <div class="row">
                                        <div
                                            class="col-12 mb-2 {{ old('type', $attribute->type) == 'select' ? '' : 'hide' }}">
                                            <div class="control-group">
                                                ...
                                                <select class="form-control select2" id="swatch_type" name="swatch_type">
                                                    <option value="dropdown"
                                                        {{ old('swatch_type', $attribute->swatch_type) == 'dropdown' ? 'selected' : '' }}>
                                                        {{ __('Dropdown') }}
                                                    </option>

                                                    {{-- <option value="color"
                                                        {{ old('swatch_type', $attribute->swatch_type) == 'color' ? 'selected' : '' }}>
                                                        {{ __('Color Swatch') }}
                                                    </option>

                                                    <option value="image"
                                                        {{ old('swatch_type', $attribute->swatch_type) == 'image' ? 'selected' : '' }}>
                                                        {{ __('Image Swatch') }}
                                                    </option>

                                                    <option value="text"
                                                        {{ old('swatch_type', $attribute->swatch_type) == 'text' ? 'selected' : '' }}>
                                                        {{ __('Text Swatch') }}
                                                    </option> --}}
                                                </select>
                                            </div>
                                        ...
                                        <div class="col-12 mb-2 hide">
                                            <div class="form-check control-group">
                                                <input class="form-check-input" type="checkbox" id="default-null-option"
                                                    name="default-null-option" value="1"
                                        ...
                                        <div class="table options_table">
                                            <table>
                                                <thead>
                                                    <tr>
                                                        <th id="swatch-header"
                                                            class="{{ old('swatch_type', $attribute->swatch_type) == 'color' || old('swatch_type', $attribute->swatch_type) == 'image' ? '' : 'hide' }}">
                                                            {{ __('Swatch') }}</th>
                                                ...
                                                <tbody id="options_list">
                                                    @if ($options = old('options', $attribute->options))

                                                        @foreach ($options as $id => $row)
                                                            <tr id="{{ $id }}">
                                                                <td
                                                                    class="swatch-picker @if (old('type', $attribute->type) != 'select' || old('swatch_type', $attribute->swatch_type) != 'color') hide @endif">
                                                                    <div class="control-group">
                                                                        <input type="color"
                                                                            name="options[{{ $id }}][color_swatch_value]"
                                                                            value="{{ isset($row['color_swatch_value']) ? $row['color_swatch_value'] : $row['swatch_value'] }}" />
                                                                    </div>
                                                                </td>

                                                                <td
                                                                    class="file @if (old('type', $attribute->type) != 'select' || old('swatch_type', $attribute->swatch_type) != 'image') hide @endif">
                                                                    <div class="control-group">
                                                                        @if (isset($row['swatch_value_url']))
                                                                            <img style="width: 36px;height: 36px;vertical-align: middle;background: #F2F2F2;border-radius: 2px;margin-right: 10px;"
                                    ...
                                    <div class="col-6 mb-2">
                                        ...
                                        <div class="form-check control-group">
                                            <input class="form-check-input" type="checkbox" id="is_unique"
                                                ...
                                                {{ old('is_unique', $attribute->is_unique) ? 'checked' : '' }}>
                                        ...
                                        <div class="control-group">
                                            ...
                                            <select class="form-control select2 category_type" id="validation"
                                                ...
                                                <option value="numeric"
                                                    {{ old('validation', $attribute->validation) == 'numeric' ? 'selected' : '' }}>
                                                    {{ __('Number') }}
                                                </option>
                                                <option value="email"
                                                    {{ old('validation', $attribute->validation) == 'email' ? 'selected' : '' }}>
                                                    {{ __('Email') }}</option>
                                                <option value="decimal"
                                                    {{ old('validation', $attribute->validation) == 'decimal' ? 'selected' : '' }}>
                                                    {{ __('Decimal') }}
                                                </option>
                                                <option value="url"
                                                    {{ old('validation', $attribute->validation) == 'url' ? 'selected' : '' }}>
                                                    {{ __('Url') }}
                                                </option>
                                            </select>
                                        </div>
                        ...
                        <section>
                            ...
                            <div class="section-body">
                                <div class="row">
                                    <div class="col-6 mb-2">
                                        <div class="form-check control-group">
                                            <input class="form-check-input" type="checkbox" id="value_per_user"
                                                name="value_per_user" value="1"
                                                {{ old('value_per_user', $attribute->value_per_user) ? 'checked' : '' }}>
                                    ...
                                    <div class="col-6 mb-2">
                                        <div class="form-check control-group">
                                            <input class="form-check-input" type="checkbox" id="is_configurable"
                                                name="is_configurable" value="1"
                                    ...
                                    <div class="col-6 mb-2">
                                        <div class="form-check control-group">
                                            <input class="form-check-input" type="checkbox" id="is_visible_on_front"
                                                name="is_visible_on_front" value="1"
...
@push('scripts')
    <script>
        ...
        var optionRows = [];
        var show_swatch = @json(old('type', $attribute->type) == 'select');
        var type = @json(old('type', $attribute->type));
        var swatch_type = @json(old('swatch_type', $attribute->swatch_type));
